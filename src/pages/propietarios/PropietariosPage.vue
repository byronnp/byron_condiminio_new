<template>
  <q-page class="propietarios-page">
    <AppListPageShell
      ...
      title="Propietarios"
      subtitle="Gestion de propietarios, contactos y asignacion de unidades."
      search-placeholder="Buscar propietario..."
...
type OwnerRow = { id: number; name: string; email: string; units: string; status: 'Activo' | 'Inactivo' | 'Pendiente'; initials: string };
type SortOption = 'recent' | 'oldest' | 'name';

const search = ref('');
const statusFilter = ref<'Todos' | 'Activo' | 'Inactivo' | 'Pendiente'>('Todos');
const sortBy = ref<SortOption>('recent');
const rowsPerPageOptions = [10, 20, 50] as const;
const pagination = ref({ page: 1, rowsPerPage: 10 });

const rows: OwnerRow[] = [
  { id: 1, name: 'Carlos Perez', email: 'carlos.perez@mail.com', units: '3 unidades', status: 'Activo', initials: 'CP' },
  { id: 2, name: 'Maria Gonzalez', email: 'maria.gonzalez@mail.com', units: '1 unidad', status: 'Activo', initials: 'MG' },
  { id: 3, name: 'Juan Rodriguez', email: 'juan.rodriguez@mail.com', units: '2 unidades', status: 'Pendiente', initials: 'JR' },
  { id: 4, name: 'Ana Torres', email: 'ana.torres@mail.com', units: '4 unidades', status: 'Activo', initials: 'AT' },
  { id: 5, name: 'Luis Ramirez', email: 'luis.ramirez@mail.com', units: '1 unidad', status: 'Inactivo', initials: 'LR' },
  { id: 6, name: 'Sofia Castillo', email: 'sofia.castillo@mail.com', units: '2 unidades', status: 'Activo', initials: 'SC' },
  { id: 7, name: 'Diego Viteri', email: 'diego.viteri@mail.com', units: '1 unidad', status: 'Activo', initials: 'DV' },
  { id: 8, name: 'Paola Mena', email: 'paola.mena@mail.com', units: '2 unidades', status: 'Pendiente', initials: 'PM' },
  { id: 9, name: 'Ricardo Vega', email: 'ricardo.vega@mail.com', units: '3 unidades', status: 'Activo', initials: 'RV' },
  { id: 10, name: 'Jorge Almeida', email: 'jorge.almeida@mail.com', units: '1 unidad', status: 'Inactivo', initials: 'JA' },
  { id: 11, name: 'Karla Cedeno', email: 'karla.cedeno@mail.com', units: '2 unidades', status: 'Activo', initials: 'KC' },
  { id: 12, name: 'Fernando Ibarra', email: 'fernando.ibarra@mail.com', units: '1 unidad', status: 'Activo', initials: 'FI' },
];

const columns = [
  { name: 'owner', label: 'Propietario', field: 'name', align: 'left' as const },
  { name: 'units', label: 'Unidades', field: 'units', align: 'left' as const },
...
const statsBase = [
  { label: 'Total propietarios', value: '12', hint: 'Propietarios registrados', icon: 'groups' },
  { label: 'Activos', value: '9', hint: 'Con unidades asociadas', icon: 'verified' },
  { label: 'Pendientes', value: '2', hint: 'Datos incompletos', icon: 'schedule' },
  { label: 'Inactivos', value: '1', hint: 'Sin actividad reciente', icon: 'block' },
];
...
const filteredRows = computed(() => {
  ...
  return rows.filter((row) => {
    ...
    const matchesQuery =
      !query || row.name.toLowerCase().includes(query) || row.email.toLowerCase().includes(query) || row.units.toLowerCase().includes(query);
...
function statusTone(status: OwnerRow['status']) {
...
<style scoped>
.propietarios-page { min-height: 100%; }
...
.type-badge,.status-badge { font-weight: 700; }
