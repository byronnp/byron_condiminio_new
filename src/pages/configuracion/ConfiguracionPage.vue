      ...
      <template #table>
        ...
        >
          ...
          <template #body-cell-category="props"
            ><q-td :props="props"
              ><q-badge outline color="primary" class="type-badge">{{ props.value }}</q-badge></q-td
            ></template
          >
...
type SettingRow = {
  ...
  detail: string;
  category: string;
  status: 'Activo' | 'Revisar' | 'Bloqueado';
  initials: string;
};
type SortOption = 'recent' | 'oldest' | 'name';
const search = ref('');
const statusFilter = ref<'Todos' | 'Activo' | 'Revisar' | 'Bloqueado'>('Todos');
...
const rows: SettingRow[] = [
  {
    id: 1,
    setting: 'Moneda base',
    detail: 'Definicion general del sistema',
    category: 'General',
    status: 'Activo',
    initials: 'MB',
  },
  {
    id: 2,
    setting: 'Impuestos',
    detail: 'Parametros de facturacion',
    category: 'Finanzas',
    status: 'Activo',
    initials: 'IM',
  },
  {
    ...
    detail: 'Alertas por correo y app',
  ...
  },
  {
    id: 4,
    setting: 'Roles',
    detail: 'Permisos y acceso por modulo',
    category: 'Seguridad',
    status: 'Activo',
    initials: 'RO',
  },
  {
    id: 5,
    setting: 'Numeracion',
    detail: 'Formato de codigos internos',
    category: 'General',
    status: 'Activo',
    initials: 'NU',
  },
  ...
  },
  {
    id: 7,
    setting: 'Backup',
    detail: 'Respaldo automatico diario',
    category: 'Seguridad',
    status: 'Activo',
    initials: 'BA',
  },
  ...
  },
  {
    id: 11,
    setting: 'Bitacora',
    detail: 'Auditoria y trazabilidad',
    category: 'Seguridad',
    status: 'Activo',
    initials: 'BI',
  },
  {
    id: 12,
    setting: 'Aprobaciones',
    detail: 'Flujos de validacion',
    category: 'Procesos',
    status: 'Activo',
    initials: 'AP',
  },
...
const columns = [
  { name: 'setting', label: 'Ajuste', field: 'setting', align: 'left' as const },
  { name: 'category', label: 'Categoria', field: 'category', align: 'left' as const },
...
const filteredRows = computed(() => {
  ...
  return rows.filter((row) => {
    ...
    const matchesQuery =
      !query ||
      row.setting.toLowerCase().includes(query) ||
...
const sortedRows = computed(() => {
  ...
  if (sortBy.value === 'name') return source.sort((a, b) => a.setting.localeCompare(b.setting));
...
function statusTone(status: SettingRow['status']) {
