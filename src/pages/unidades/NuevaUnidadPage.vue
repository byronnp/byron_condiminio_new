      ...
      <div v-else class="content-grid">
        <q-form ref="formRef" class="form-column" @submit.prevent="submitForm">
          <section class="context-panel">
            ...
            <div>
              <span>Condominio seleccionado</span><strong>{{ condominiumName }}</strong>
            </div>
          </section>
            ...
            <q-card-section>
              <div class="section-heading">
                ...
                <div>
                  ...
                  ><span>Define dónde se encuentra y cómo reconocerla.</span>
                </div>
              </div>
              <div class="form-grid q-mt-lg">
                <q-skeleton v-if="loadingBlocks" type="QInput" />
                <q-select
                  v-else-if="blockOptions.length"
                ...
                <q-input
                  ...
                  @update:model-value="suggestCode"
                />
                <q-input
                  v-model="form.code"
                  dense
                  ...
                  maxlength="40"
                  :rules="[requiredTextRule]"
                  @update:model-value="codeWasEdited = true"
                />
              </div>
              <div v-if="!loadingBlocks && blocksLoadError" class="options-error q-mt-md">
                <q-icon name="error_outline" /><span>{{ blocksLoadError }}</span
                ><q-btn flat dense no-caps label="Reintentar" @click="loadBlocks" />
              </div>
              <div
                v-else-if="!loadingBlocks && !blockOptions.length"
                class="empty-options-note q-mt-md"
              >
                <q-icon name="info_outline" /><span
                  >La casa se registrará sin bloque asignado.</span
                >
              </div>
            </q-card-section>
          </q-card>
            ...
            <q-card-section>
              <div class="section-heading">
                ...
                <div>
                  ...
                  ><span>Configura únicamente los datos propios de la casa.</span>
                </div>
              </div>
                ...
                <q-item tag="label"
                  ><q-item-section
                    ...
                    ><q-item-label caption
                      >Habilita propietarios, residentes y responsables de
                      facturación.</q-item-label
                    ></q-item-section
                  ><q-item-section side><q-toggle v-model="form.isAssignable" /></q-item-section
                ></q-item>
                <q-separator />
                <q-item tag="label"
                  ><q-item-section
                    ...
                    ><q-item-label caption
                      >Permite utilizarla en las operaciones del condominio.</q-item-label
                    ></q-item-section
                  ><q-item-section side><q-toggle v-model="form.isActive" /></q-item-section
                ></q-item>
              </div>
            </q-card-section>
          </q-card>
          ...
          <div class="form-actions">
            <q-btn flat no-caps label="Cancelar" @click="goBack" /><q-btn
              color="primary"
              unelevated
              no-caps
              icon="add_home"
          ...
          <q-card flat bordered class="summary-card">
            <q-card-section>
              ...
              <div class="summary-list">
                <div>
                  <span>Condominio</span><strong>{{ condominiumName }}</strong>
                </div>
                <div>
                  <span>Número</span><strong>{{ form.number || 'Sin definir' }}</strong>
                </div>
                <div v-if="form.blockId !== null">
                  <span>Sector</span><strong>{{ selectedBlockName }}</strong>
                </div>
                <div>
                  <span>Área</span><strong>{{ form.areaM2 || 0 }} m²</strong>
                </div>
                <div>
                  <span>Asignable</span><strong>{{ form.isAssignable ? 'Sí' : 'No' }}</strong>
                </div>
                <div>
                  <span>Estado</span
              ...
              <q-banner rounded class="next-step q-mt-lg"
                >Después podrás agregar personas y parqueaderos desde el detalle.</q-banner
              >
            </q-card-section>
          </q-card>
        </aside>
