<template>
  <q-page class="houses-page">
    ...
    >
      <template #stats><AppStatsCards :cards="stats" /></template>
      <template #table>
        <q-banner v-if="!condominiumId" rounded class="context-banner q-mb-md" role="status"
          ><template #avatar><q-icon name="apartment" /></template>Selecciona un condominio en el
          layout para consultar sus casas.</q-banner
        >
        <q-banner v-else-if="error" rounded class="error-banner q-mb-md" role="alert"
          ><template #avatar><q-icon name="error_outline" color="negative" /></template
          >{{ error }}</q-banner
        >
        <q-banner v-else rounded class="page-meta q-mb-md" role="status"
          ><template #avatar><q-icon name="dataset" /></template>{{ pageMeta }}</q-banner
        >
        <q-table
          ...
          <template #body-cell-code="props"
            ><q-td :props="props"
              ><div class="house-cell">
                <q-avatar rounded size="38px" class="house-avatar"><q-icon name="home" /></q-avatar>
                <div>
          ...
          <template #body-cell-assignable="props"
            ><q-td :props="props"
              ><q-badge
                ...
                >{{ props.row.isAssignable ? 'Asignable' : 'No asignable' }}</q-badge
              ></q-td
            ></template
          >
          <template #body-cell-active="props"
            ><q-td :props="props"
              ><q-badge
                ...
                >{{ props.row.isActive ? 'Activa' : 'Inactiva' }}</q-badge
              ></q-td
            ></template
          >
          <template #body-cell-actions="props"
            ><q-td :props="props" class="table-actions"
              ...
              ><q-btn flat round dense icon="edit" class="table-icon" disable
                ><q-tooltip>Edición pendiente de soporte del API</q-tooltip></q-btn
              ></q-td
            ></template
          >
          <template #no-data
            ><div class="empty-state">
              <q-icon name="home_work" size="38px" /><strong>No hay casas para mostrar</strong
              ><span>{{
                hasFilters
      ...
      <template #body-cell-block="props">
        <q-td :props="props"
          ><div class="table-primary">{{ props.row.blockName }}</div></q-td
        >
      </template>
      ...
      <template #body-cell-people="props">
        <q-td :props="props"
          ><div class="count-cell">
            <q-icon name="group" size="17px" /><strong>{{ props.row.peopleCount }}</strong>
          </div></q-td
        >
      </template>
      <template #body-cell-parking="props">
        <q-td :props="props"
          ><div class="count-cell">
            <q-icon name="local_parking" size="17px" /><strong>{{ props.row.parkingCount }}</strong>
          </div></q-td
        >
      </template>
      <template #footer
