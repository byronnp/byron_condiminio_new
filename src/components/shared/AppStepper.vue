...
function getStepAriaLabel(step: AppStepperItem, index: number) {
  ...
  return step.description
    ? `Paso ${index + 1}: ${label}. ${step.description}`
    : `Paso ${index + 1}: ${label}`;
}
...
.app-stepper__step {
  ...
  position: relative;
  transition:
    border-color var(--app-nav-transition),
    box-shadow var(--app-nav-transition),
    transform var(--app-nav-transition),
    background-color var(--app-nav-transition),
    color var(--app-nav-transition);
  width: auto;
}
...
.app-stepper__step--active {
  border-color: var(--app-nav-border-active);
  box-shadow: var(--app-nav-shadow-active);
