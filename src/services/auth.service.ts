...
function normalizeCondominium(item: unknown): CondoOption | null {
  ...
  return { id, name, city, units, active };
}
...
function resolveUser(payload: unknown, fallbackEmail: string): ApiUser {
  ...
  const user = { ...dataUserRecord, ...userRecord };
  ...
  const platformAdminValue =
    user.is_platform_admin ?? record.is_platform_admin ?? dataRecord.is_platform_admin;
...
async function requestJson<T>(
  ...
  return { response, data: await parseJsonResponse<T>(response), unauthorized };
}
async function hydrateUserSession(
  ...
  return {
    ...
    activeCondoId:
      resolveUserRole(userFromPayload) === 'senior'
        ? null
...
export async function activateAccess(payload: ActivateAccessPayload): Promise<void> {
  ...
  if (!response.ok) {
    ...
    const message =
      extractServiceErrorMessage(data) ?? mapActivateAccessFallbackMessage(response.status);
